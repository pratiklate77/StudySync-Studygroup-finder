...
class KafkaRetryWorker:
    ...
    async def run(self) -> None:
        ...
        while not self._stop_event.is_set():
            ...
            if success:
                logger.info("Retried event successfully event_id=%s retries=%d", event.event_id, event.retry_count)
                continue
            event.retry_count += 1
            delay = min(self._base_delay * (2 ** max(0, event.retry_count - 1)), self._max_delay)
            logger.warning("Retry failed event_id=%s retry_count=%d next_in=%.2fs", event.event_id, event.retry_count, delay)
    ...
    async def stop(self) -> None:
        ...
        await self._fallback_store.put(EventEnvelope(topic="__retry_worker_shutdown__", key=b"", value={}))
